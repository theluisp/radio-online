...
function select_datos($tblname,$field_name,$field_id){
	...
	function GetFavoritesByCatid($cookiecrisp){
		$query = $this->db->query("select * from favourite join station on station.id = favourite.station_id where favourite.user_id = $cookiecrisp and favourite.status=1");
		$query->execute();
		$data = $query->fetchAll();
		return $data;
	}
    
    function getFavStaion($cookiecrisp){
		$query = $this->db->query("select * from favourite join station on station.id = favourite.station_id where favourite.user_id = $cookiecrisp");
		$query->execute();
		$data = $query->fetchAll();
		return $data;
	}
